...
export const UsuariosManager = () => {
  ...
  const cambiarEstado = async (id, isActive, username) => {
    ...
    const confirmar = window.confirm(
      `Estas seguro de que queres ${accion} al usuario ${username}?`
    );
    ...
    try {
      ...
      if (data.ok) {
        ...
        alert(data.message || `No se pudo ${accion} el usuario.`);
      }
    } catch (error) {
      console.error(`Error al ${accion}:`, error);
      alert(error.message || `No se pudo ${accion} el usuario.`);
    }
  };
    ...
    <div>
      <div className="mb-4">
        ...
        <p style={{ color: "var(--color-text-secondary)" }}>
          Administra los clientes registrados en la plataforma, sus estados y
          accesos.
        </p>
      </div>
      ...
      {loading ? (
        <div
          className="text-center py-5 bg-white rounded shadow-sm border"
        ...
        >
          ...
          <p className="mt-3 text-muted">No hay usuarios registrados todavia.</p>
        </div>
      ) : (
            ...
            >
              ...
              >
                ...
                <div className="overflow-hidden">
                  ...
                  <small
                    className="text-uppercase mt-1 d-block"
                    style={{ fontSize: "0.75rem", letterSpacing: "0.5px" }}
                  >
                    {usuario.role === "admin" || usuario.role === "superadmin" ? (
                      <span className="text-dark fw-bold">
                        <i className="bi bi-shield-lock text-warning"></i> ADMIN
                      </span>
                    ) : (
              ...
              <div
                className="d-flex align-items-center justify-content-between w-100 mt-2 mt-md-0 gap-3"
                style={{ flex: 1 }}
              >
                <div>
                ...
                <div className="d-flex justify-content-end">
                  {!canManageUser(usuario) ? (
                    <button
                      ...
                      disabled
                      title="No podes modificar la cuenta de otro administrador"
                    >
